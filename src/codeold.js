...
export default function Whitehat(props) {
  ...
  const mapGroupSelection = useMemo(() => {
    ...
    if (
      ...
      const popCounts = Object.values(statePopulations).map(
        (entry) => entry.population
      );
      console.log("Pop Counts   :", popCounts);
      ...
      const [popMin, popMax] = d3.extent(popCounts);
      ...
      /*
      const getPopulationByAbbreviation = (abbreviation) => {
        const stateInfo = props.populationData.find(
          (state) => state.abbreviation === abbreviation
        );
        return stateInfo ? stateInfo.population : null;
      };
      */

      function getPopulationByAbbreviation(abbreviation) {
        console.log("Abb 1", abbreviation);
        const stateInfo = props.populationData.find(
          (s) => s.stateName === abbreviation
        );

        if (stateInfo) {
          console.log(stateInfo.population);
          return stateInfo.population;
        }

        console.log("Abb 2", abbreviation);
      ...
      mapGroup
        ...
        .on("mouseover", (e, d) => {
          ...
          let abbreviation = sname;

          let gunDeathsPer100K =
            (count * 100000) /
            getPopulationByAbbreviation(abbreviation).toFixed(2);
